...
<div class="modal fade" id="ModalEditar<?= $id  ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<?= form_open_multipart('medicos/editar/'.$id.'/'.$foto, array('id' => 'medicFormEditar'.$id)) ?>
			...
			<div class="modal-body bg-info">
				...
				<div class="row">
					...
					<div class="col-xs-6">
						<?= form_dropdown('especialidad', $especialidades, $especialidad, array('id' => 'especialidad')) ?>
					</div>
				</div>
				<div class="row">
					<div class="col-xs-6">
					...
					<div class="col-xs-6">
						<?= form_input(array('type' => 'date','name' => 'ingreso','id' => 'ingreso', 'value' => $ingreso)) ?>
					</div>
				</div>
				<div class="row">
					<div class="col-xs-6">
					...
					<div class="col-xs-6">
						<?= form_input('consultorio', $consultorio, array('id' => 'consultorio', 'maxlength' => 50)) ?>
					</div>
				</div>
				<div class="row">
					<div class="col-xs-6">
					...
					<div class="col-xs-6">
						<?= form_input(array('type' => 'number','name' => 'precio_consulta','id' => 'precio_consulta', 'min' => '1', 'value' => $precio_consulta)) ?>
					</div>
				</div>
			</div>
...
<script>
	$(function(){
		...
		$("#medicFormEditar<?= $id ?>").submit(function(){
			var formData = new FormData(document.getElementById("medicFormEditar<?= $id ?>"));
            $.ajax({
                url: "<?= base_url() ?>medicos/editar/<?= $id ?>/<?= $foto ?>",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
