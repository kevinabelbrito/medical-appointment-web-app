...
<div class="modal fade" id="ModalEditar<?= $id  ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<?= form_open('especialidades/editar/'.$id, array('id' => 'espFormEditar'.$id)) ?>
			...
			<div class="modal-body bg-info">
				...
				<div class="row">
					<div class="col-xs-6">
						<?= form_label('Descripción', 'descripcion'.$id) ?>
					</div>
					<div class="col-xs-6">
						<?= form_input('descripcion', $descripcion, array('id' => 'descripcion'.$id, 'maxlength' => 50)) ?>
					</div>
				</div>
			</div>
...
<script>
	$(function(){
		...
		$("#espFormEditar<?= $id ?>").submit(function(){
			var formData = new FormData(document.getElementById("espFormEditar<?= $id ?>"));
            $.ajax({
                url: "<?= base_url() ?>especialidades/editar/<?= $id ?>",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
