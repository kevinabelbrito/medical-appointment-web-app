<div class="modal fade" id="ModalEditar<?= $id  ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<?= form_open_multipart('usuarios/editar/'.$id.'/'.$foto, array('id' => 'userFormEditar'.$id)) ?>
...
<script>
	$(function(){
		...
		$("#userFormEditar<?= $id ?>").submit(function(){
			var formData = new FormData(document.getElementById("userFormEditar<?= $id ?>"));
            $.ajax({
                url: "<?= base_url() ?>usuarios/editar/<?= $id ?>/<?= $foto ?>",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
